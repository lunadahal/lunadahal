Hi, I am Luna Dahal — M.S. Cybersecurity student focused on application security and web security testing. Currently exploring for Application Security Full-time Roles.

*Skills*: 
- Burp Suite, Python, OWASP Web Top 10, OWASP LLM Top 10, HTTP Traffic Analysis, Prompt Injection, SAST, DAST, Git, GitHub, VS Code, Wireshark

*Certifications*:
- Shields Up Cybersecurity Job Simulation, Forage (AIG) 2026  
- Udacity Introduction to Cybersecurity Nanodegree


*Projects*:

[HumanVsAI-WebSecurity](https://github.com/lunadahal/HumanVsAI-WebSecurity-) — Compared AI vs human HTTPs traffic using Burp Suite (DAST), and documented 4 security findings, and 
demonstrated a live prompt injection attack mapped to OWASP LLM Top 10 LLM01.

[P1 Joe's Auto Security](https://github.com/lunadahal/P1-joes-auto-security) — Windows security hardening and access control.

[P2 DFI Environment Monitoring](https://github.com/lunadahal/P2-dfi-environment-monitoring-and-securing) — Firewall analysis, IDS rules, incident response.

[P3 Hospital Incident Response](https://github.com/lunadahal/P3-hospital-incident-response-plan) — Ransomware scenario, vulnerability scanning.

[P4 SwiftTech GRC](https://github.com/lunadahal/P4-swifttech-grc) — Risk assessment, HIPAA and NIST alignment.



*Contact*: [LinkedIn](https://www.linkedin.com/in/luna-dahal-75a481222/) | lunadahal5@gmail.com
