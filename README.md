👋 Hi, I'm **Luna Dahal**  
Cybersecurity student exploring defensive security, ethical hacking, and security tools through hands-on labs and projects.
Passionate about learning how attacks work — and how to stop them.

TECHNICAL SKILLS
Security & Frameworks: NIST CSF, CIS Controls, NIST SP 800-53, Risk Assessment, Access Control, Endpoint Security, System Hardening
Tools & OS: Windows OS, Windows Defender Antivirus, PowerShell, Windows Services, Group Policy
Data & Analytics: SQL, Excel, Azure Power BI, Tableau

- 🎓 **M.S. Cybersecurity**, Lindsey Wilson College (Expected Dec 2027)  
- 🎓 **B.S. Management Information Systems**, University of Texas at Arlington (2020)  
- 🎓 **Udacity Nanodegree – Introduction to Cybersecurity**

**Professional Experience**
- **System Analyst – Tech Intelli**  
  •	Applied root cause analysis to troubleshoot and resolve critical system outage within 2 hours, identifying Azure DevOps pipeline configuration error and restoring service
 •	Analyzed complex datasets using Excel and Azure Power BI to identify risks and recommend strategic initiatives, contributing to $150,000 annual revenue increase
 •	Designed system architecture diagrams communicating technical solutions to cross-functional teams
  
- **System Analyst – Tekvana Inc**  
  •	Implemented automated testing with Selenium and JIRA integration, accelerating development cycle and enabling faster developer feedback
  •	Developed PL/SQL packages (stored procedures, functions, triggers) reducing manual data entry by 40% and improving process efficiency by 20%
  •	Built SQL and Tableau dashboards tracking product KPIs, reducing manual reporting by 8 hours weekly

🔗 **LinkedIn:** https://www.linkedin.com/in/luna-dahal-75a481222/
📧 **Email:** lunadahal5@gmail.com
